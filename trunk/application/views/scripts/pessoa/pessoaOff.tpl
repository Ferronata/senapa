{*
 * Pessoa => View de manipulação de dados da classe 'Pessoa'
 * Data de Cricação - 17/05/2010
 * @author Leonardo Popik e João Marcos=> Classgen 1.0
 * @version 1.0
 *}
				<div class="content">
					<input type="hidden" id="id" name="id" value="{$object->getId()}" />
					<div class="line">
						<label class="label" for="nome">Nome</label>
						<div class="innerLine">
							<input type="text" disabled="disabled" class="input grande" id="nome" name="nome" value="{$object->getNome()}" />
						</div>
					</div>
					<div class="line">
						<label class="label" for="email">E-Mail</label>
						<div class="innerLine">
							<input type="text" class="input grande" id="email" name="email" value="{$object->getEmail()}" />
						</div>
					</div>
					<div class="line">
						<label class="label" for="site">Site</label>
						<div class="innerLine">
							<input type="text" class="input grande" id="site" name="site" maxlength="250" value="{$object->getSite()}" />
						</div>
					</div>
				</div>